...
const ProfilePaymentTypeComponent = (props) => {
  const [input, setInput] = useState('');
  const [name, setName] = useState('계좌 이체');
    ...
    <>
      <ProfileNameWrapper>
        <div className='profile-name-update-save'>
          <h2 className='profile-name-field'>결제 수단</h2>

          <div className='profile-action-group'>
            <div className={shouldHideUpdate ? 'hidden' : 'update-button'} onClick={updateButtonHandler}>
              수정
            </div>
            <div className={shouldHideSave ? 'hidden' : 'save-button'} onClick={saveButtonHandler}>
              저장
            </div>
          </div>
        </div>
        <div className='info'>
          ...
          <div className={shouldHideInput ? 'hidden' : 'input-message-wrapper'}>
            <textarea placeholder={name} value={input} onChange={handleInput} className='form-control-is-valid' />
          </div>
          <div className={shouldHideError ? 'hidden' : 'invalid-feedback'}>소개는 80자를 넘을 수 없습니다.</div>
        </div>
      </ProfileNameWrapper>
    </>
  );
...
export default ProfilePaymentTypeComponent;
